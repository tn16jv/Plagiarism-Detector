...
namespace Plagiarism_Engine.Services
{
    ...
    {
        /// <summary>
        /// Converts a file to the abstract syntax tree using the cpp14 grammar. It first creates a parse tree then converts that to an AST
        /// </summary>
        /// <param name="f">the file to convert</param>
        /// <returns>the root node of the AST</returns>
       public static Node getCppASTForUser(File f)
        {
            ...
            Node root = new Node(CPP14Parser.ruleNames[ctx.RuleIndex], ctx.Start.Line, "", f.Id);
        ...
        public static Node parseToAST(ParserRuleContext ctx, bool verbose, int indentation, Node par)
        {
            ...
            if (!toBeIgnored)
            {
                String ruleName = CPP14Parser.ruleNames[ctx.RuleIndex];
                Node temp;
                if (ruleName.Equals("unqualifiedid"))
        ...
        /// <summary>
        /// Takes a token stream for a file then splits it up by method. splits it up by functiondefinition rule
        /// </summary>
        ...
        public static List<List<Token>> SplitMethods(List<Token> list)
        {
            ...
            var indexlist = list.Where(x => x.val == "functiondefinition").Select(x => list.IndexOf(x)).ToList();
